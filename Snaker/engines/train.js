...
function genRandCode(url){
	var code = $.recognize(url,true); 
...
function genLoginRandCode(){
	lcode = genRandCode("https://dynamic.12306.cn/otsweb/passCodeAction.do?rand=lrand");
	return lcode;
}

function genOrderRandCode(){
	pcode = genRandCode("https://dynamic.12306.cn/otsweb/passCodeAction.do?rand=randp");
	return pcode;
}
...
function queryTrain(trainNumber){
	...
	for(i=0;i<alltrains.length;++i){
		...
		if(train.value.indexOf(trainNumber)!=-1){
			return train.id;
		}
	}
	return null;
...
function bookTicket(){
	var url = "https://dynamic.12306.cn/otsweb/order/querySingleAction.do?method=submutOrderRequest";
	var param = {};
	param.arrive_time=trainInfo.arriveTime;
	param.from_station_name = $.from;
	param.from_station_telecode= getStationCode($.from);
	param.from_station_telecode_name = $.from;
	param.include_student="00";
	param.lishi=trainInfo.lishi;
	param.round_start_time_str="00:00--24:00";
	param.round_train_date=getCurrentDate();
	param.single_round_type='1';
	param.start_time_str="00:00--24:00";
	param.station_train_code=$.trainNumber;
	param.to_station_name=$.to;
	param.to_station_telecode=getStationCode($.to);
	param.to_station_telecode_name=$.to;
	param.train_class_arr="QB#D#Z#T#K#QT#";
	param.train_date=$.date;
	param.train_pass_type="QB";
	param.train_start_time=trainInfo.startTime;
	param.trainno=trainCode;
	param.ypInfoDetail=trainInfo.ypInfoDetail;
	var response=$.post(url,param,"utf8");
	var body = response.body;
	$.print(body);
}

function getToken(){
	var url = "https://dynamic.12306.cn/otsweb/order/querySingleAction.do?method=init";
	var response = $.get(url);
	var body = response.body;
	var tag = body.indexOf("org.apache.struts.taglib.html.TOKEN");
	if(tag!=-1){
		tag = body.indexOf('value="',tag);
		if(tag!=-1){
			tag = tag + 'value="'.length;
			var endTag = body.indexOf('"',tag);
			var token = body.substring(tag,endTag);
...
var messageRegex = /var\s*message\s*=\s*\"([^\"]*)\"/m;

function submitOrder(){
	var token = getToken();
	var url = "https://dynamic.12306.cn/otsweb/order/confirmPassengerAction.do?method=confirmPassengerInfoSingle";
	var param={};
	param.checkbox0='0';
	param.checkbox9='Y';
	param.checkbox9='Y';
	...
	param.checkbox9='Y';
	param.checkbox9='Y';
	param.oldPassengers=$.trueName+",1,"+$.peopleId;//殷山,1,340821198003123015
	param["orderRequest.bed_level_order_num"]="000000000000000000000000000000";
	param["orderRequest.cancel_flag"]="1"
	param["orderRequest.end_time"]=trainInfo.arriveTime;
	param["orderRequest.from_station_name"]=$.from;//南京
	param["orderRequest.from_station_telecode"]=getStationCode($.from);//NJH
	param["orderRequest.id_mode"]="Y";
	param["orderRequest.reserve_flag"]="A";
	param["orderRequest.seat_type_code"]="";	
	param["orderRequest.start_time"]=trainInfo.startTime;
	param["orderRequest.station_train_code"]=$.trainNumber;//K8586
	param["orderRequest.ticket_type_order_num"]="";	
	param["orderRequest.to_station_name"]=$.to;//桐城
	param["orderRequest.to_station_telecode"]=getStationCode($.to);//TTH
	param["orderRequest.train_date"]=$.date;//2012-01-13
	param["orderRequest.train_no"]=trainCode;//55000K858610
	param["org.apache.struts.taglib.html.TOKEN"]=token;//c477551c277322c6a51ab5eb4d5c513d
	param["passengerTickets"]="1,1,"+$.trueName+",1,"+$.peopleId+","+$.telephone+",Y";//1,1,殷山,1,340821198003123015,13151561122,Y
	param.passenger_1_cardno=$.peopleId;//340821198003123015
	param.passenger_1_cardtype="1";
	param.passenger_1_mobileno=$.telephone;//13151561122
	param.passenger_1_name=$.trueName;//殷山
	param.passenger_1_seat="1";//	1
	param.passenger_1_ticket="1";//	1
	param.randCode=pcode;//
	param.textfield="中文或拼音首字母";
	var response=$.post(url,param,"utf8");
	var body= response.body;
	$.print(body);
	var allmsg = body.match(messageRegex);
	if(allmsg!=null){
		var msg = "";
		if(allmsg.length>=2){
			msg = allmsg[1];
